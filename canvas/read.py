...
class CanvasReader(object):
    ...
    def get_all_courses(self):
        return self.api.get('/accounts/1/courses')
    
    def get_courses_for_subaccount(self, name):
        for subaccount in self.api.get('/accounts/1/sub_accounts', parameters={"recursive": True}):
            if subaccount['name'] == name:
                courses = self.api.get('/accounts/%s/courses' % subaccount['id'])
                return courses
    ...
    def get_sections(self, course_id):
        parameters = {'include':'students'}
        return self.api.get('/courses/%s/sections' % course_id, parameters=parameters)
